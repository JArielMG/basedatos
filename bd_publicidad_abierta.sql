...
CREATE ALGORITHM=UNDEFINED DEFINER=`patepjf`@`%` SQL SECURITY DEFINER VIEW `vact_ejercicios`  AS  select `cat_ejercicios`.`id_ejercicio` AS `id_ejercicio`,`cat_ejercicios`.`ejercicio` AS `ejercicio`,`cat_ejercicios`.`active` AS `active` from `cat_ejercicios` where (`cat_ejercicios`.`active` = 1) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`patepjf`@`%` SQL SECURITY DEFINER VIEW `vlimite_act`  AS  select `vlimite`.`total` AS `limite` from `vlimite` order by `vlimite`.`total` limit 1 ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`patepjf`@`%` SQL SECURITY DEFINER VIEW `vmonto_campana`  AS  select `b`.`id_campana_aviso` AS `id_campana_aviso`,sum(`b`.`monto_desglose`) AS `monto` from (`tab_facturas` `a` join `tab_facturas_desglose` `b`) where (`a`.`id_factura` = `b`.`id_factura`) group by `b`.`id_campana_aviso` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`patepjf`@`%` SQL SECURITY DEFINER VIEW `vout_campana_lugar`  AS  select `a`.`id_campana_lugar` AS `ID de lugar`,`a`.`id_campana_aviso` AS `ID de campana o aviso institucional`,`a`.`poblacion_lugar` AS `Lugar` from `rel_campana_lugar` `a` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`patepjf`@`%` SQL SECURITY DEFINER VIEW `vtop10_campanas`  AS  select `a`.`id_campana_aviso` AS `id_campana_aviso`,sum(`a`.`monto_desglose`) AS `sum(a.monto_desglose)` from `vact_facturas_desglose` `a` group by `a`.`id_campana_aviso` order by sum(`a`.`monto_desglose`) desc limit 10 ;
